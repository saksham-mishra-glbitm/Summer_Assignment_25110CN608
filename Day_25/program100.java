...
public class program100
{
    ...
    public static void main(String[] sm)
    {
        ...
        quicksort(names,0,n-1);        //sort by length
    ...
    static int partition(String[] arr,int low, int high)        //partition using length
    {
        String pivot = arr[high];
        int i=low-1;
        for(int j=low;j<high;j++)
        {
            if(arr[j].length() < pivot.length())        //compare by length
            {
                i++;
                String temp=arr[i];
                arr[i]=arr[j];
